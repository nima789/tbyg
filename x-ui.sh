...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/nima789/tbyg/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontents.com/nima789/tbyg/main/x-ui.sh
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/nima789/tbyg/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
