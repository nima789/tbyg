...
red "不支持你当前系统，请选择使用Ubuntu,Debian,Centos系统。请向作者反馈 https://github.com/nima789/tbyg@yg-warp/issues" && rm -f CFwarp.sh && exit 1
fi
...
yellow "有疑问请向作者反馈 https://github.com/nima789/tbyg@yg-warp/issues"
exit 0
else 
...
[[ $cpu = AMD64 ]] && wget -N https://cdn.jsdelivr.net/gh/nima789/tbyg@yg-warp/wgcf_2.2.14_amd64 -O /usr/local/bin/wgcf && chmod +x /usr/local/bin/wgcf         
[[ $cpu = ARM64 ]] && wget -N https://cdn.jsdelivr.net/gh/nima789/tbyg@yg-warp/wgcf_2.2.14_arm64 -O /usr/local/bin/wgcf && chmod +x /usr/local/bin/wgcf
if [[ $main -lt 5 || $minor -lt 6 ]] || [[ $vi =~ lxc|openvz ]]; then
[[ -e /usr/bin/wireguard-go ]] || wget -N https://cdn.jsdelivr.net/gh/nima789/tbyg@yg-warp/wireguard-go -O /usr/bin/wireguard-go && chmod +x /usr/bin/wireguard-go
fi
echo | wgcf register
...
2 ) WARPup;;
3 ) wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/nima789/tbyg@yg-warp-plus/wp.py && python3 wp.py;;
4 )
wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/nima789/tbyg@yg-warp-plus/wp.py
sed -i "27 s/[(][^)]*[)]//g" wp.py
...
5 ) wget -N https://raw.githubusercontents.com/nima789/tbyg/yg-acme-script/screen.sh && bash screen.sh && back;;
0 ) bash CFwarp.sh
esac
}
...
screen -d >/dev/null 2>&1
wget -N --no-check-certificate https://raw.githubusercontents.com/nima789/tbyg/yg-warp//WARP-CR.sh
readp "输入国家区域简称（例：新加坡，输入大写SG;美国，输入大写US）:" gj
